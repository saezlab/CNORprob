...
optRound_optim    <- 5        # rounds of optimisation
...
MaxTime           <- 600      # time for each round of optimisation [seconds]
HLbound           <- 0.5      # cut-off for high and low weights
SSthresh          <- 2e-16    # cut-off for states' difference at steady-state
printCost         <- 1        # print or not print intermediate fitting cost [0,1]
...
# tol               <- 1e-5     # relative tol. for optim. / default = 1e-8
trace             <- 1        # print objfunc every iter / default = 1

# Post-optimisation analysis
Analyses          <- c(F,F,F,F) # [F,T] edge knockout, node knockout, sensitivity analysis
...
if (Selected_Model==1) {
  ...
  ModDatppProb <- preprocessing_Prob(CNOlist, pknmodel, expansion=FALSE,compression=FALSE, cutNONC=FALSE, verbose=FALSE) # Now use CNORprob specific pre-processing (expansion=FALSE by default and report)
...
# plotModel(bString$model,CNOlist,round(bString$bString,digits=2))
# dev.off()
...
estim_based <- estim_original; if (Analyses[3]) { estim_Result  <- CNORprob_LPSA(optmodel,optCNOlist,estim_based,res,HLbound,LPSA_Increments) }
...
estim$BS_Round <- BS_Round
estim_original <- estim
estim_based <- estim_original; if (Analyses[4]) { estim_Result  <- CNORprob_BS(model,CNOlist,estim_based,res,BS_Type,BS_Round) }
