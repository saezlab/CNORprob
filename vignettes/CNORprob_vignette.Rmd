...
# source("Rscripts/CNORprob_Rscripts.R")
...
Analyses          <- c(F,F,F) # [F,T] edge knockout, node knockout, sensitivity analysis
...
if (Analyses[1]) { estim_Result  <- CNORprob_edgeKO(model,CNOlist,estim,res) }
if (Analyses[2]) { estim_Result  <- CNORprob_nodeKO(model,CNOlist,estim,res) }
if (Analyses[3]) { estim_Result  <- CNORprob_LPSA(model,CNOlist,estim,res,HLbound,LPSA_Increments) }
